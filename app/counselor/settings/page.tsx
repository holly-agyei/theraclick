...
import {
  LogOut, Save, ChevronRight, Camera, Lock, Mail,
  Bell, MessageSquare, Users, Moon, Sun, Type,
  HelpCircle, Flag, Phone, Trash2, AlertTriangle, Shield, Download,
  ExternalLink, Upload, X, Briefcase,
} from "lucide-react";
...
function SettingsRow({ icon: Icon, iconBg, iconColor, label, description, right, onClick, destructive }: {
  ...
  description?: string; right?: React.ReactNode; onClick?: () => void; destructive?: boolean;
}) {
...
export default function CounselorSettingsPage() {
  ...
  const initials = profile?.fullName?.split(" ").map((n: string) => n[0]).join("").slice(0, 2).toUpperCase() || "C";
  ...
  const completion = useMemo(() => {
    ...
    const items: { done: boolean; label: string }[] = [
      ...
      { done: !!(profile.application?.specialization), label: "Add specialization" },
      { done: !!(profile.application?.about), label: "Write your bio" },
    ];
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      await updateDoc(doc(db, "users", profile.uid), {
        "application.specialization": specialization.trim(),
        "application.about": about.trim(),
      });
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !profile) { if (fileInputRef.current) fileInputRef.current.value = ""; return; }
    if (!file.type.startsWith("image/")) { alert("Please select an image file"); if (fileInputRef.current) fileInputRef.current.value = ""; return; }
    if (file.size > 5 * 1024 * 1024) { alert("Image must be under 5MB"); if (fileInputRef.current) fileInputRef.current.value = ""; return; }

    let fileToUpload = file;
    if (file.size > 400 * 1024) {
      try { fileToUpload = await compressImageUtil(file, { targetSizeBytes: 400 * 1024 }); }
      catch { alert("Image compression failed. Use a smaller image."); if (fileInputRef.current) fileInputRef.current.value = ""; return; }
    }
    ...
    try {
      ...
      if (db) await updateDoc(doc(db, "users", profile.uid), { avatar: imageUrl, profilePicture: imageUrl });
    } catch { alert("Upload failed. Try a smaller image."); }
    finally { setUploadingPhoto(false); if (fileInputRef.current) fileInputRef.current.value = ""; }
  };

  const handleRemovePhoto = async () => {
    if (!profile || !db) return;
    try { await updateDoc(doc(db, "users", profile.uid), { avatar: null, profilePicture: null }); setProfilePicture(""); }
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      setCurrentPassword(""); setNewPasswordVal(""); setConfirmPasswordVal("");
      setTimeout(() => setShowPasswordForm(false), 2000);
    } catch (err: unknown) {
      const code = (err as { code?: string })?.code;
      if (code === "auth/wrong-password" || code === "auth/invalid-credential") setPasswordStatus({ type: "error", msg: "Current password is incorrect." });
      else if (code === "auth/too-many-requests") setPasswordStatus({ type: "error", msg: "Too many attempts. Try again later." });
      else setPasswordStatus({ type: "error", msg: "Failed to update password. Try again." });
  ...
  const handleChangeEmail = async () => {
    ...
    try {
      ...
      if (db && profile) await updateDoc(doc(db, "users", profile.uid), { email: newEmail.trim() });
      setEmailStatus({ type: "success", msg: "Email updated successfully!" });
      setNewEmail(""); setEmailPassword("");
      setTimeout(() => setShowEmailForm(false), 2000);
    } catch (err: unknown) {
      const code = (err as { code?: string })?.code;
      if (code === "auth/wrong-password" || code === "auth/invalid-credential") setEmailStatus({ type: "error", msg: "Password is incorrect." });
      else if (code === "auth/email-already-in-use") setEmailStatus({ type: "error", msg: "This email is already in use." });
      else if (code === "auth/requires-recent-login") setEmailStatus({ type: "error", msg: "Please sign out and sign back in, then try again." });
      else setEmailStatus({ type: "error", msg: "Failed to update email. Try again." });
  ...
  const handleDeleteAccount = async () => {
    ...
    try { await deleteDoc(doc(db, "users", profile.uid)); await logout(); }
        ...
        <div className="px-4 py-6 pb-28 md:px-8 md:py-10">
          <div className="mx-auto max-w-lg">
            ...
            <div className="mb-2 rounded-2xl bg-white dark:bg-gray-950 p-4 shadow-sm">
              <div className="flex items-center gap-4">
                <div className="relative">
                  {profilePicture ? (
                    <img src={profilePicture} alt="Profile"
                      className="h-14 w-14 rounded-full object-cover ring-2 ring-[#2BB5A0] ring-offset-2 dark:ring-offset-gray-950" />
                  ) : (
                  ...
                  <button onClick={() => setEditingPhoto(!editingPhoto)}
                    className="absolute -bottom-1 -right-1 flex h-6 w-6 items-center justify-center rounded-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm hover:bg-gray-50">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-gray-400 capitalize">{profile?.role || "Counselor"}</p>
                </div>
              </div>
                ...
                <div className="mt-4 rounded-xl bg-gray-50 dark:bg-gray-900 p-3">
                  <div className="flex items-center gap-4">
                    <div className="relative shrink-0">
                      {profilePicture ? (
                        <div className="relative">
                          <img src={profilePicture} alt="Profile" className="h-16 w-16 rounded-full object-cover border-2 border-gray-200 dark:border-gray-800" />
                          <button onClick={handleRemovePhoto} className="absolute -top-1 -right-1 rounded-full bg-red-500 p-0.5 hover:bg-red-600">
                            <X className="h-3 w-3 text-white" />
                          </button>
                        </div>
                      ) : (
                        <div className="h-16 w-16 rounded-full bg-[#0F4F47] flex items-center justify-center text-lg font-bold text-white border-2 border-gray-200 dark:border-gray-800">{initials}</div>
                      )}
                    </div>
                    <div className="flex-1">
                      <input ref={fileInputRef} type="file" accept="image/*" onChange={handlePhotoUpload} className="hidden" />
                      <button disabled={uploadingPhoto} onClick={() => fileInputRef.current?.click()}
                        className="rounded-xl border border-gray-200 dark:border-gray-700 px-3 py-2 text-[13px] font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 disabled:opacity-50">
                        <Upload className={`mr-1.5 inline h-3.5 w-3.5 ${uploadingPhoto ? "animate-pulse" : ""}`} />
                        {uploadingPhoto ? "Uploading..." : "Upload Photo"}
                      </button>
                      <p className="mt-1 text-[11px] text-gray-400">JPG, PNG or GIF. Max 5MB.</p>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="rounded-2xl bg-white dark:bg-gray-950 shadow-sm overflow-hidden divide-y divide-gray-100 dark:divide-gray-800">
              <SettingsRow icon={Briefcase} iconBg="bg-orange-100 dark:bg-orange-900/30" iconColor="text-orange-500"
                label="Edit Profile" description={specialization || "Add your specialization"}
              ...
              {editingProfile && (
                <div className="px-4 py-4">
                  <div className="space-y-3">
                    <div>
                      <label className="mb-1.5 block text-[12px] font-medium text-gray-500">Specialization</label>
                    ...
                    <div>
                      <label className="mb-1.5 block text-[12px] font-medium text-gray-500">About</label>
                      <textarea value={about} onChange={(e) => setAbout(e.target.value)}
                        placeholder="Tell students about your background and approach..."
                        rows={3}
                        className="w-full resize-none rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 px-3 py-2.5 text-[14px] text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:border-[#2BB5A0] focus:outline-none" />
                    </div>
                    <div className="flex gap-2 pt-1">
                      ...
                      <button onClick={() => { setEditingProfile(false); setSpecialization(profile?.application?.specialization || ""); setAbout(profile?.application?.about || ""); }}
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <Input type="password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)}
                        placeholder="Enter current password" className="border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 text-[14px]" />
                    </div>
                    <div>
                      ...
                      <Input type="password" value={newPasswordVal} onChange={(e) => setNewPasswordVal(e.target.value)}
                        placeholder="At least 6 characters" className="border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 text-[14px]" />
                    </div>
                    <div>
                      ...
                      <Input type="password" value={confirmPasswordVal} onChange={(e) => setConfirmPasswordVal(e.target.value)}
                        placeholder="Re-enter new password" className="border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 text-[14px]" />
                    </div>
                    {passwordStatus && (
                      <p className={`text-[13px] font-medium ${passwordStatus.type === "success" ? "text-[#2BB5A0]" : "text-red-500"}`}>{passwordStatus.msg}</p>
                    )}
                    <div className="flex gap-2 pt-1">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <Input type="email" value={newEmail} onChange={(e) => setNewEmail(e.target.value)}
                        placeholder="your-new-email@example.com" className="border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 text-[14px]" />
                    </div>
                    <div>
                      ...
                      <Input type="password" value={emailPassword} onChange={(e) => setEmailPassword(e.target.value)}
                        placeholder="Enter your current password to confirm" className="border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 text-[14px]" />
                    </div>
                    {emailStatus && (
                      <p className={`text-[13px] font-medium ${emailStatus.type === "success" ? "text-[#2BB5A0]" : "text-red-500"}`}>{emailStatus.msg}</p>
                    )}
                    <div className="flex gap-2 pt-1">
            ...
            <div className="rounded-2xl bg-white dark:bg-gray-950 shadow-sm overflow-hidden divide-y divide-gray-100 dark:divide-gray-800">
              ...
              <SettingsRow icon={MessageSquare} iconBg="bg-sky-100 dark:bg-sky-900/30" iconColor="text-sky-500"
                label="New messages" description="Student messages and booking requests"
