...
import { collection, getDocs, query, orderBy, doc, getDoc, limit, onSnapshot, where, or } from "firebase/firestore";
...
export default function PeerMentorInboxPage() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(collection(db, "directMessages"), async (snap) => {
      ...
      for (const convDoc of snap.docs) {
        ...
        // Check if this conversation involves the peer mentor
        if (participants && Array.isArray(participants) && participants.includes(profile.uid)) {
          const studentId = participants.find(p => p !== profile.uid);

          if (studentId && !seenStudentIds.has(studentId) && db) {
            ...
            try {
              ...
              console.error("Error fetching student data for inbox:", e);
            }
          }
        }
      }
              ...
              <div className="space-y-3">
                {filteredConversations.map((conv) => (
                  <button
                    ...
                    className="group w-full rounded-xl border border-white/10 bg-white/5 p-5 text-left transition-all hover:border-emerald-500/30 hover:bg-white/10"
                  >
