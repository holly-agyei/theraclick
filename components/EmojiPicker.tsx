...
export function EmojiPicker({ onSelect, className = "" }: EmojiPickerProps) {
  ...
  return (
    <div ref={pickerRef} className={`relative ${className}`}>
      ...
      {/* Picker dropdown */}
      {isOpen && (
        <div className="absolute bottom-12 left-0 z-50 w-[280px] rounded-2xl border border-gray-200
          bg-white shadow-xl animate-in fade-in slide-in-from-bottom-2 duration-200">
          {/* Category tabs */}
            ...
            {emojiCategories[activeTab].emojis.map((emoji) => (
              <button
                ...
                className="flex h-8 w-8 items-center justify-center rounded-lg text-lg
                  hover:bg-gray-100 active:scale-90 transition-transform"
              >
                {emoji}
              </button>
            ))}
